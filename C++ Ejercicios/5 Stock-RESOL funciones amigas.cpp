...
class Producto {
    ...
    public:
        ...
        void friend ingresar (char * , char *, int , int);
        void friend venderUno (char *, char*);
        void friend imprimirMarcasConProductosAReponer();
        void friend crearProd (char *, int , int , Marca *);
        void friend verStock ();
...
void Comercio::ingresar (char * marca, char * mod, int stock, int prepo){
    ...
    while (MPUNT){
        if (!stricmp (MPUNT->nombre,marca)){
            crearProd (mod, stock, prepo, MPUNT);
            return;
        }
        if (!(MPUNT->sig)){
            M =  new Marca (marca);
